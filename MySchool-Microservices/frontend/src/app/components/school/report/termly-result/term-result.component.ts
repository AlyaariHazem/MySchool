...
export class TermResultComponent implements OnInit {
  ...
  private prepareHeaders(): void {
    this.subjectNames = [
      ...new Set(this.allReports.flatMap((r) => r.gradeSubjects.map((s) => s.subjectName)))
    ];
  ...
  getGradeForSubjectType(subjects: GradeSubject[], subjectName: string, type: string): number {
